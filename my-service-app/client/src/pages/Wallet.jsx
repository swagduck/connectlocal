...
import { Wallet as WalletIcon, History, CreditCard, Plus, Trash2, XCircle } from 'lucide-react'; // Import XCircle
...
const Wallet = () => {
    ...
    // 👇 Hàm hủy giao dịch nạp tiền
    const handleCancelTransaction = async (id) => {
        if (!window.confirm("Bạn có chắc muốn hủy giao dịch nạp tiền này?")) return;

        try {
            await api.put(`/transactions/${id}/cancel`);
            toast.success("Đã hủy giao dịch nạp tiền");
            // Cập nhật lại danh sách
            setTransactions(transactions.map(t => t._id === id ? { ...t, status: 'cancelled' } : t));
        } catch (error) {
                                                ...
                                                <tr key={tx._id} className="border-b last:border-0 hover:bg-gray-50 transition group">
                                                    ...
                                                    ${tx.status === 'completed' ? 'bg-green-100 text-green-700' :
                                                                tx.status === 'failed' ? 'bg-red-100 text-red-700' :
                                                                    tx.status === 'cancelled' ? 'bg-gray-100 text-gray-700' : 'bg-yellow-100 text-yellow-700'}
                                                `}>
                                                            {tx.status === 'completed' ? 'Thành công' :
                                                                tx.status === 'failed' ? 'Thất bại' :
                                                                    tx.status === 'cancelled' ? 'Đã hủy' : 'Chờ xử lý'}
                                                        </span>
                                                    </td>
